...
dataset_val = dataset.iloc[100:].to_csv('output_dir/val2.csv')
...
def save_config(key, value):
    data = {}
    if os.path.exists(path + 'configs.json'):
        with open(path + 'configs.json') as json_file:
            data = json.load(json_file)

    data[key] = value
    with open(path + 'configs.json', 'w') as outfile:
